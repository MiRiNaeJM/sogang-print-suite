"""Client의 프로그램 정보 창을 구성하는 Tkinter 대화상자 모듈.

Manager에서 배포된 aboutContent를 주입받아 GitHub, 라이선스, 매뉴얼 정보를 사용자에게 보여준다.
"""
...
class ToolTip:
    """아이콘이나 버튼 위에 짧은 도움말을 띄우기 위한 보조 창."""
    ...
    def move(self, event=None) -> None:
        """도움말 창이 마우스 포인터를 따라가도록 위치를 갱신한다."""
        if self.tip is None:
            return
...
class AboutDialog(tk.Toplevel):
    ...
    def __init__(self, master: tk.Misc, content: Mapping[str, Any] | None = None) -> None:
        ...
        self.content = normalize_about_content(content) if content is not None else load_about_content()
    ...
    def open_github(self) -> None:
        ...
        if github_url and "OWNER/REPOSITORY" not in github_url:
            ...
            messagebox.showinfo("GitHub", "GitHub 주소를 about_content.json 또는 about_content.py에서 수정하세요.", parent=self)
