"""Client 리소스 파일을 안전하게 로드하는 유틸리티 모듈.

PyInstaller 실행 환경과 개발 환경 모두에서 아이콘 파일을 찾고, 실패해도 앱 실행을 막지 않는다.
"""
...
def _set_window_icon(window: tk.Misc, image_ref_holder: object | None = None) -> tk.PhotoImage | None:
